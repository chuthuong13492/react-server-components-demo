...
export function Router() {
  ...
  const locationKey = useMemo(() => JSON.stringify(location), [location.selectedId, location.isEditing, location.searchText]);
  ...
  const refresh = useCallback(function refresh(response) {
    startTransition(() => {
  ...
  const navigate = useCallback(function navigate(nextLocation) {
    startTransition(() => {
      setLocation(loc => {
        const merged = {
          ...loc,
        ...
        if (
          merged.selectedId === loc.selectedId &&
          merged.isEditing === loc.isEditing &&
          merged.searchText === loc.searchText
        ) {
          return loc;
        }
        return merged;
  ...
  const contextValue = useMemo(() => ({ location, navigate, refresh }), [location, navigate, refresh]);
